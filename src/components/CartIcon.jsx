...
const MINICART_DISABLED_PREFIXES = ["/checkout", "/order-confirmation"];
...
const CartIcon = ({
  ...
  const isMinicartDisabled = MINICART_DISABLED_PREFIXES.some(
  ...
  useEffect(() => {
    if (!openFromAddCart || !isDesktop || isMinicartDisabled) return;
  ...
  }, [openFromAddCart, isDesktop, isMinicartDisabled, setOpenFromAddCart]);

  // ── Hover: open MiniCart (desktop only) ──
  const handleMouseEnter = useCallback(() => {
    if (!isDesktop || isMinicartDisabled) return;
  ...
  return (
    ...
    >
      ...
      {/* MiniCart: only render on desktop, and never on checkout/order-confirmation */}
      {isDesktop && !isMinicartDisabled && (
        <MiniCart
          isOpen={isOpen}
