...
const WishlistPage = () => {
  ...
  // Render guard: only items backed by a real product. Drops malformed/stale
  // store entries (e.g. a legacy localStorage row, or any orphan) so they can
  // never paint a broken card with no image, name or price.
  const validWishlist = wishlist?.filter((item) => item?.product?.id) ?? [];
